/*
	Custom configs for Al Rayak.
	modified by JDubb18T

	All of these configs exist in the main config. The configs below will simply override any config from the main config (although the majority of them are the same).
	Explanations to all of these configs also exist in the main config.
*/

DMS_findSafePosBlacklist append
[
	[]		// Salt flats are blacklisted for Altis by default.
];

// These configs are the default values from the main config. Just included here as an example.
DMS_PlayerNearBlacklist				= 1000;
DMS_SpawnZoneNearBlacklist			= 1000;
DMS_TraderZoneNearBlacklist			= 2000;
DMS_MissionNearBlacklist			= 1000;
DMS_WaterNearBlacklist				= 400;

// Altis is pretty flat, so we can make the min surfaceNormal ... stricter? more strict? Who cares, you get the idea.
DMS_MinSurfaceNormal				= 0.65;


// Making these configs below as strict as possible will help in reducing the number of attempts taken to find a valid position, and as a result, improve performance.

DMS_MinDistFromWestBorder			= 500;	// There's at least 2km of ocean from the west edge to the first bit of land.
DMS_MinDistFromEastBorder			= 500;	// There's over 2km of ocean from the east edge to the first bit of land.
DMS_MinDistFromSouthBorder			= 500;	// There's about 5km of ocean from the south edge to the first bit of land.
DMS_MinDistFromNorthBorder			= 500;	// There's around 5km of ocean from the north edge to the first bit of land (including the island).


// Add the "saltflats" and "slums" mission to the existing mission types.
DMS_StaticMissionTypes append 			[
                                            //["OelAuge",1],
                                            //["falloutisland",1],
                                            //["falloutbridge",1],
											//["occupation",1],
                                            //["schlachthaus",1],
	                                        //["GuantanamoBay",1]
											//["Reg44",1],
											//["MartinFob",1],
											//["LamboParty",1],
											["DasLager",1],
											["DieBaustelle",1],
											["WaffenFabrik",1]
];

// Add the "salt flats base" and "slums" to the "bases" to spawn on server startup. NOTE: "append" and "pushback" are NOT the same.
//DMS_BasesToImportOnServerStart append ["falloutisland","falloutbridge"];
